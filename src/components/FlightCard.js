...
const FlightCard = ({ flight, searchParams, onClick, selected }) => {
  ...
  const formatDateTime = (dt) =>
    new Date(dt).toLocaleString([], { hour: '2-digit', minute: '2-digit', hour12: true });
  ...
  return (
    ...
    >
      <CardHeader
        avatar={
          legs && legs[0] && legs[0].carriers?.marketing?.[0] ? (
            ...
            <Avatar sx={{ width: 56, height: 56 }}>{price.formatted.charAt(0)}</Avatar>
          )
        }
        title={
        ...
        <CardContent>
          {farePolicy && (
            <Box sx={{ mb: 2, backgroundColor: '#f5f5f5', p: 1, borderRadius: 1 }}>
              ...
              <Typography variant="body2" color="textSecondary">
                Change Allowed: {farePolicy.isChangeAllowed ? 'Yes' : 'No'} | Partially Changeable:{' '}
                {farePolicy.isPartiallyChangeable ? 'Yes' : 'No'} | Cancellation Allowed:{' '}
                {farePolicy.isCancellationAllowed ? 'Yes' : 'No'} | Partially Refundable:{' '}
          ...
          {legs && legs.map((leg, legIndex) => {
            ...
            return (
              ...
              >
                ...
                <Box sx={{ mt: 1 }}>
                  ...
                  <Stack direction="row" spacing={1} alignItems="center">
                    ...
                    <Typography variant="body2">
                      ...
                      {leg.timeDeltaInDays > 0 && ` (+${leg.timeDeltaInDays} day)`}
                    </Typography>
                  </Stack>
                  <Stack direction="row" spacing={1} alignItems="center">
                ...
                {leg.segments && leg.segments.map((segment, segIndex) => {
                  const segDeparture = formatDateTime(segment.departure);
                  const segArrival = formatDateTime(segment.arrival);
                  const segDurationHours = Math.floor(segment.durationInMinutes / 60);
                    ...
                    >
                      <Stack direction="row" spacing={1} alignItems="center">
                        ...
                        <Typography variant="subtitle2">
                          Segment {segIndex + 1} (Flight {segment.flightNumber})
                        </Typography>
                      </Stack>
                      <Typography variant="body2" sx={{ mt: 1 }}>
                        {segment.origin.name} ({segment.origin.displayCode}) → {segment.destination.name} ({segment.destination.displayCode})
                      </Typography>
                      <Stack direction="row" spacing={1} alignItems="center" sx={{ mt: 1 }}>
                        <FlightTakeoffIcon fontSize="small" color="action" />
                        <Typography variant="body2"><strong>Dep:</strong> {segDeparture}</Typography>
                        <FlightLandIcon fontSize="small" color="action" />
                        <Typography variant="body2"><strong>Arr:</strong> {segArrival}</Typography>
                      </Stack>
                      <Typography variant="body2">
                        <strong>Duration:</strong> {segDurationHours}h {segDurationMinutes}m
                      </Typography>
                      {carrier && (
                        <Typography variant="body2">
      ...
      {searchParams && (
        <Box sx={{ mt: 2, p: 2, border: '1px dashed #aaa', borderRadius: 2, backgroundColor: '#fffde7' }}>
          <Typography variant="body2" color="textSecondary">
            <strong>Cabin Class:</strong> {searchParams.cabinClass.charAt(0).toUpperCase() + searchParams.cabinClass.slice(1)}
          </Typography>
          <Typography variant="body2" color="textSecondary">
            <strong>Passengers:</strong> {searchParams.adults} Adult(s), {searchParams.childrens} Child(ren), {searchParams.infants} Infant(s)
          </Typography>
